...
class AclImdbDateset(Dataset):
    ...
    def _get_file_info(self):
        ...
        for dir_tmp in self.data_dir_list:
            ...
            self.file_path_list = [os.path.join(dir_tmp, file_name) for file_name in self.file_name_list if file_name.endswith("txt")]
...
if __name__ == "__main__":
    ...
    train_set = AclImdbDateset(root_dir, vocab_path, is_train=True, max_len=200)
    valid_set = AclImdbDateset(root_dir, vocab_path, is_train=True, max_len=200)

    train_loader = DataLoader(dataset=train_set, batch_size=10, shuffle=True)
    for i, (inputs, target) in enumerate(train_loader):
        print(i, input.shape, inputs, target.shape, target)
